...
public class L_0000_Assistant {
    ...
    public static void createJavaFileFromWeb() throws Exception {
        String url = prepareUrl();
        JSONObject json = getQuestionJsonFromWeb(url);
        Map<String, String> questionInfo = parseQuestionJSON(json, url);
    ...
    private static void createJavaFileFromLocal() throws Exception {
        String url = prepareUrl();
        JSONObject json = getQuestionJsonFromLocal(url);
        Map<String, String> questionInfo = parseQuestionJSON(json, url);
        String className = String.format("L_%S_%s", questionInfo.get(QI_ID), questionInfo.get(QI_TITLE).replaceAll(" ", "_"));
        StringBuilder modelString = readModelFromFile(MODEL_FILENAME);
        String javaString = replaceHoder(modelString, className, questionInfo);
    ...
    private static String prepareUrl() throws IOException {
    ...
    private static JSONObject getQuestionJsonFromWeb(String url) throws IOException {
    ...
    private static JSONObject getQuestionJsonFromLocal(String url) {
        String title = url.substring(url.indexOf("problem") + "problem".length() + 1, url.indexOf("description") - 1);
        // 根据标题获取问题的唯一信息如 ID 等
        String uniqueUrl = "https://www.lintcode.com/api/problems/detail/?unique_name_or_alias=%title%&_format=detail";
        uniqueUrl = uniqueUrl.replace("%title%", title);
        Out.p("点击下方地址，然后将信息拷入：");
        Out.p(uniqueUrl);
    ...
    private static Map<String, String> parseQuestionJSON(JSONObject json, String url) {
        // 解析信息到 map
        Map<String, String> map = new HashMap<>();
        map.put(QI_BASEURL, url);
        map.put(QI_ID, String.format("%04d", json.getInt("id")));
        for (String field : new String[]{QI_TITLE, QI_DESC, QI_EXAMP, QI_CHAL, QI_DESC}) {
            StringBuilder sb = new StringBuilder();
            for (String line : json.getString(field).split("\n")) {
                sb.append(" *      ").append(line).append("\n");
            }
            String value = sb.substring(" *      ".length(), sb.length() - 1);
            map.put(field, value);
        }
...
//        createJavaFileFromWeb();
        createJavaFileFromLocal();
    }
